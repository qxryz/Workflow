...
enum ModelRegistrationPresetRegistry {
    static func providerSpecificRegistrations(
        for model: ModelConfig,
        provider: ProviderConfig?,
        status: RegistrationStatus = .unverified
    ) -> [ModelRegistration]? {
        guard let provider,
              ProviderEndpointCatalog.normalizedProviderName(provider.name) == "agnes" else {
            return nil
        }
        let templates = ProviderInterfaceTemplateRegistry.recommended(providerKey: provider.name, modelId: model.modelId)
        guard !templates.isEmpty else { return nil }
        return templates.map { template in
            var registration = RegisteredModelInterface(template: template, model: model, provider: provider)
            registration.status = status
            registration.lastTestSummary = "Ready from Agnes AI model template"
            registration.lastModifiedByUser = false
            return registration
        }
    }

    static func draft(for model: ModelConfig, provider: ProviderConfig?) -> ModelRegistration {
        if let registration = providerSpecificRegistrations(for: model, provider: provider, status: .unverified)?.first {
            return registration
        }
        return make(model: model, provider: provider, task: inferredTask(for: model), presetKey: "migrated.\(model.endpointKind.rawValue)")
    }
