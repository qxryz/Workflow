...
enum ProviderInterfaceTemplateRegistry {
    static let templateVersion = "2026.06.05.2"
    static let checkedAt = "2026-06-05"
    ...
    private static let agnesAI: [ProviderInterfaceTemplate] = [
        ...
        make(
            ...
            path: "/videos",
            mode: .async,
            defaultRequestJSON: #"{"mode":"ti2vid","height":768,"width":1152,"num_frames":121,"frame_rate":24}"#,
            inputCards: [
                prompt("prompt"),
            ...
            outputSlots: [
                taskOutput("id|data.id|task_id|data.task_id"),
                assetOutput("Video", .video, "video_url|output.video_url|data.video_url|data.output.video_url"),
                assetOutput("Completed Video", .video, "remixed_from_video_id|output.remixed_from_video_id|data.remixed_from_video_id|data.output.remixed_from_video_id")
            ],
            polling: polling(
                taskIdPath: "id|data.id|task_id|data.task_id",
                path: "/videos/{task_id}",
                statusPath: "status|data.status|output.status|data.output.status"
            )
        )
    ]
