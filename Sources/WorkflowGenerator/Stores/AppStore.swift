...
extension AppConfiguration {
    func modelSettingsTarget(for node: WorkflowNode) -> UUID? {
        if let interfaceId = node.registeredModelInterfaceId,
           let registration = modelRegistrations.first(where: { $0.id == interfaceId }),
           models.contains(where: { $0.id == registration.modelId }) {
            return registration.modelId
        }
        guard let modelId = node.modelId,
              models.contains(where: { $0.id == modelId }) else {
            return nil
        }
        return modelId
    }
...
final class AppStore {
    ...
    func applyAppearanceToChatWindows() {
        ...
        settingsWindow?.appearance = configuration.boardSettings.nsAppearance
    }

    @discardableResult
    func openModelSettings(for modelId: UUID?) -> Bool {
        guard let modelId, configuration.models.contains(where: { $0.id == modelId }) else { return false }
        settingsSelection.tab = .models
        settingsSelection.modelId = modelId
        return true
    }

    func openModelSettingsWindow(for modelId: UUID?) {
        guard openModelSettings(for: modelId) else { return }
        DispatchQueue.main.async { [weak self] in
            self?.openSettingsWindow()
        }
    }

    func openModelSettingsWindow(for node: WorkflowNode) {
        openModelSettingsWindow(for: modelSettingsTarget(for: node))
    }

    func modelSettingsTarget(for node: WorkflowNode) -> UUID? {
        configuration.modelSettingsTarget(for: node)
    }
    ...
    func openSettingsWindow() {
        ...
        let hostingController = NSHostingController(
            rootView: SettingsView(store: self)
                .frame(minWidth: 1040, idealWidth: 1120, minHeight: 700, idealHeight: 760)
                .workflowAppearance(configuration.boardSettings)
        )
        let window = NSWindow(contentViewController: hostingController)
        window.title = AppCopy(locale: configuration.language).settings
        window.setContentSize(NSSize(width: 1120, height: 760))
        window.styleMask = [.titled, .closable, .miniaturizable, .resizable]
        window.isReleasedWhenClosed = false
        window.appearance = configuration.boardSettings.nsAppearance
        window.center()
