...
enum InvocationProgress: Equatable {
    ...
    case polling(taskId: String, attempt: Int, maximum: Int)
    ...
    var message: String {
        ...
        case .polling(let taskId, let attempt, let maximum):
            "生成中（已查询 \(attempt)/\(maximum) 次）。任务 ID：\(taskId)"
        }
    }
}
...
struct InvocationExecutor {
    ...
    private func poll(
        ...
        for attempt in 1...rule.maxAttempts {
            ...
            } else if shouldEmitPollingHeartbeat(
                attempt: attempt,
                maximum: rule.maxAttempts,
                intervalSeconds: rule.intervalSeconds
            ) {
                onProgress(.polling(taskId: taskId, attempt: attempt, maximum: rule.maxAttempts))
            }
    ...
    /// Decides whether a "still polling" heartbeat should be emitted to the UI.
    /// Used to give the user feedback during long async tasks where the status
    /// string doesn't change between polls (e.g. "processing" the whole time).
    ///
    /// - Parameters:
    ///   - attempt: 1-based current attempt number (1 = first poll, right after the first interval sleep)
    ///   - maximum: total max attempts from `ModelRegistrationPolling`
    /// - Returns: `true` to emit a `.polling` callback; `false` to stay silent
    private func shouldEmitPollingHeartbeat(attempt: Int, maximum: Int, intervalSeconds: Int) -> Bool {
        guard attempt > 1 else { return false }
        if attempt == maximum { return true }
        let interval = max(intervalSeconds, 1)
        let attemptsPerHeartbeat = max(1, Int(ceil(30.0 / Double(interval))))
        return attempt.isMultiple(of: attemptsPerHeartbeat)
    }
