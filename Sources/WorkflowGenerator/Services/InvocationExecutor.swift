...
struct InvocationExecutor {
    ...
    private func poll(
        ...
        for attempt in 1...rule.maxAttempts {
            ...
            } else if shouldEmitPollingHeartbeat(attempt: attempt, maximum: rule.maxAttempts) {
    ...
    private func shouldEmitPollingHeartbeat(attempt: Int, maximum: Int) -> Bool {
        // TODO: 决定什么时候发"还在轮询"的心跳回调
        // 节奏感比精确重要。10s 间隔下 30s 一次比较舒服，不要每次都发（吵）
        // 思路：
        //   - 按 attempt 取模（每 N 次发一次）
        //   - 最后一次（attempt == maximum）建议发，告诉用户"差一点就超时"
        //   - 第一轮（attempt == 1）通常不发，避免和 .status 抢戏
        return false
    }
