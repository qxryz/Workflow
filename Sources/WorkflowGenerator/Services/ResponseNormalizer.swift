...
enum JSONPathReader {
    static func values(at path: String, in object: Any) -> [Any] {
        let fallbackPaths = path.split(separator: "|").map {
            $0.trimmingCharacters(in: .whitespacesAndNewlines)
        }
        if fallbackPaths.count > 1 {
            for fallbackPath in fallbackPaths {
                let fallbackValues = values(at: fallbackPath, in: object)
        ...
        return read(parts: path.split(separator: ".").map(String.init), object: object)
    }

    static func requiredString(at path: String, in object: Any) throws -> String {
        guard let value = values(at: path, in: object).first else {
