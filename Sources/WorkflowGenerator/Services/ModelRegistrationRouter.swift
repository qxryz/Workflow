...
struct ModelRegistrationRouter {
    ...
    private func compiledRequest(
        ...
        return CompiledInvocationRequest(
            ...
            timeoutInterval: submissionTimeout(for: context.registration)
        )
    }

    func submissionTimeout(for registration: ModelRegistration) -> TimeInterval {
        guard registration.mode == .async else { return 90 }
        switch registration.task {
        case .textToVideo, .imageToVideo, .referenceToVideo, .videoToVideo,
             .videoEdit, .textToAudioVideo, .imageToAudioVideo, .audioToAudioVideo,
             .videoToAudioVideo, .multimodalToAudioVideo:
            return 300
        default:
            return 90
        }
