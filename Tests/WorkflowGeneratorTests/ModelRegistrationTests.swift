...
final class ModelRegistrationTests: XCTestCase {
    ...
    func testAsyncExecutorReportsPollingHeartbeatForUnchangedStatus() async throws {
        let client = StubInvocationHTTPClient(responses: [
            .json(["task_id": "task-1"]),
            .json(["status": "running"]),
            .json(["status": "running"]),
        ...
        XCTAssertTrue(progress.values.contains(.polling(taskId: "task-1", attempt: 3, maximum: 4)))
    }

    func testJSONPathReaderUsesFallbackPathsAndStringifiesNumericIds() throws {
        ...
        XCTAssertEqual(try JSONPathReader.requiredString(at: "id|data.id|task_id", in: payload), "42")
        XCTAssertEqual(
            JSONPathReader.values(at: "video_url|output.video_url|data.output.video_url", in: payload).first as? String,
    ...
    func testModelSettingsTargetUsesRegisteredInterfaceWhenNodeModelIdIsMissing() throws {
        let provider = ProviderConfig(name: "OpenAI", baseURL: "https://api.openai.com/v1", apiKey: "key")
        let model = makeModel(provider: provider, modelId: "gpt-5")
        ...
        let node = WorkflowNode(
            ...
            chat: [],
            draftMessage: "",
            registeredModelInterfaceId: registration.id
        )

        XCTAssertEqual(configuration.modelSettingsTarget(for: node), model.id)
    }
    ...
    func testAgnesVideoTemplateCompilesAsyncKeyframeParameters() throws {
        ...
        XCTAssertEqual(registration.polling?.taskIdPath, "id|data.id|task_id|data.task_id")
        XCTAssertEqual(registration.polling?.statusPath, "status|data.status|output.status|data.output.status")
        XCTAssertTrue(registration.outputSlots.contains { $0.jsonPath.contains("output.video_url") })
        XCTAssertTrue(registration.outputSlots.contains { $0.jsonPath.contains("remixed_from_video_id") })
    }

    func testAgnesVideoTemplateSendsTextToVideoDefaultsAndUsesLongSubmitTimeout() throws {
        let provider = ProviderConfig(name: "Agnes AI", baseURL: "https://apihub.agnes-ai.com/v1", apiKey: "key")
        let model = makeModel(provider: provider, modelId: "agnes-video-v2.0")
        let template = try XCTUnwrap(ProviderInterfaceTemplateRegistry.template(id: "agnes.video"))
        var registration = RegisteredModelInterface(template: template, model: model, provider: provider)
        ...
        let payload = try ModelRegistrationRouter().compiledPayload(
            interfaceId: registration.id,
            inputs: [InvocationAsset(type: .text, text: "Generate a short video.")],
            configuration: configuration
        )

        XCTAssertEqual(payload["model"] as? String, "agnes-video-v2.0")
        XCTAssertEqual(payload["prompt"] as? String, "Generate a short video.")
        XCTAssertEqual(payload["mode"] as? String, "ti2vid")
        XCTAssertEqual(payload["height"] as? Int, 768)
        XCTAssertEqual(payload["width"] as? Int, 1152)
        XCTAssertEqual(payload["num_frames"] as? Int, 121)
        XCTAssertEqual(payload["frame_rate"] as? Int, 24)
        XCTAssertEqual(ModelRegistrationRouter().submissionTimeout(for: registration), 300)
    }
